...
This project demonstrates the deployment and management of applications using Kubernetes on a local environment with Minikube. It includes deploying an application, exposing it as a service, scaling it, and inspecting its logs.
...
Docker Desktop: For container runtime support on your system.
...
Defines the deployment for an application with 2 replicas:
...
Defines a NodePort service to expose the application:
```bash
yaml
